...
while left_cap.isOpened() and right_cap.isOpened():
    ...
    # Нормалізація та кольорова карта
    disparity_map_normalized = cv2.normalize(disparity_map, None, 0, 255, cv2.NORM_MINMAX)
    disparity_map_colored = cv2.applyColorMap(np.uint8(disparity_map_normalized), cv2.COLORMAP_JET)

    # Відображення
    cv2.imshow("Ліве зображення", left_frame)
    cv2.imshow("Праве зображення", right_frame)
    cv2.imshow("Карта глибин", disparity_map_colored)
