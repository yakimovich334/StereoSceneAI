...
for filename in image_files:
    ...
    # Нормалізація для візуалізації з використанням нелінійного масштабування
    ...
    # Застосовуємо кольорову палітру для карти диспаратності
    disparity_map_colored = cv2.applyColorMap(disparity_map_normalized, cv2.COLORMAP_JET)
    ...
    cv2.imshow("Карта диспаратності", disparity_map_colored)
    cv2.waitKey(1000)
...
# Створення відео з карт диспаратності
print("Створення відео з карт диспаратності...")
disparity_files = sorted([f for f in os.listdir(output_dir) if f.startswith('disparity_')])

if disparity_files:
    # Завантажуємо перше зображення для визначення розміру
    first_frame = cv2.imread(os.path.join(output_dir, disparity_files[0]))
    height, width = first_frame.shape[:2]

    # Налаштування відеофайлу
    video_output_path = os.path.join(output_dir, 'disparity_video.mp4')
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Кодек для MP4
    fps = 10  # Кількість кадрів за секунду
    video_writer = cv2.VideoWriter(video_output_path, fourcc, fps, (width, height))

    # Додаємо всі карти диспаратності до відео
    for disparity_file in disparity_files:
        frame = cv2.imread(os.path.join(output_dir, disparity_file))
        video_writer.write(frame)

    # Завершуємо створення відео
    video_writer.release()
    print(f"Відео збережено як: {video_output_path}")
else:
    print("Не знайдено карт диспаратності для створення відео.")
